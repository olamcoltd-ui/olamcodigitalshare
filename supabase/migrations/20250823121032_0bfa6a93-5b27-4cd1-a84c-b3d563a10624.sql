...
CREATE POLICY IF NOT EXISTS "Admins can upload product files"
ON storage.objects
FOR INSERT TO authenticated
WITH CHECK (bucket_id = 'product-files' AND public.is_admin());

-- Admins can update product files
CREATE POLICY IF NOT EXISTS "Admins can update product files"
ON storage.objects
FOR UPDATE TO authenticated
...
CREATE POLICY IF NOT EXISTS "Admins can delete product files"
ON storage.objects
FOR DELETE TO authenticated
USING (bucket_id = 'product-files' AND public.is_admin());